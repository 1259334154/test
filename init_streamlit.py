...
def replace_favicon(streamlit_model_path):
    """替换streamlit的icon"""
    logo_path = os.path.join(streamlit_model_path,
                             'streamlit',
                             'static',
                             'favicon.png')
    # 删除 logo
...
def init_streamlit(streamlit_model_path, title, footer):
    index_path = pathlib.Path(streamlit_model_path) /"streamlit" / "static" / "index.html"
...
init_streamlit(streamlit_model_path=streamlit_model_path,footer='Made with kourou')
